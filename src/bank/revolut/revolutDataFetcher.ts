...
export class RevolutDataFetcher {
    ...
    public async fetch(): Promise<RevolutTransactionData> {
        ...
        const transactionsResponse = await this.request.get(`/user/current/transactions?from=${fromDate.valueOf()}`);
        const walletResponse = await this.request.get(`/user/current/wallet`);
